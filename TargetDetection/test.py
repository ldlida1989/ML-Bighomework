...
from eval_test import BaseTransform
from eval_test import SIXray_CLASSES as labelmap
from eval_test import SIXrayAnnotationTransform, SIXrayDetection, SIXray_ROOT
...
def test(img_path, anno_path):
    # 图片和标签路径
    global annopath, imgpath
    annopath = os.path.join(anno_path, '%s.txt')
    imgpath = os.path.join(img_path, '%s.jpg')
    print(annopath, imgpath)
    EPOCH = 49300
    print(EPOCH)
    ...
    # 加载数据
    dataset = SIXrayDetection(anno_path, img_path, args.imagesetfile,
