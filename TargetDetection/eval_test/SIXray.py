...
class SIXrayDetection(data.Dataset):
    ...
    def __init__(self, txt_path, image_path,
        ...
        self._annopath = osp.join('%s' % self.txt_path, '%s.txt')
        self._imgpath = osp.join('%s' % self.image_path, '%s.jpg')

        self.ids = list()
        
        images = []
        #读取
        for f in os.listdir(os.path.join(self.image_path)):
            images.append(f.split('.')[0])
        with open(self.image_set, 'w') as f:
            for d in images:
                f.write('{}\n'.format(d))
        print('写入完成')
